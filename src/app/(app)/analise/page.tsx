...
import { LockedFeature } from "@/components/plans/locked-feature";
...
export default async function AnalisePage() {
  ...
  const canAdvanced = canUseFeature(user.plan, "ADVANCED_ANALYSIS");
  const canPurchase = canUseFeature(user.plan, "PURCHASE_ANALYSIS");
  const canAI = canUseFeature(user.plan, "AI_ASSISTANT");

  const [overspending, alerts, overview, goals] = await Promise.all([
    canAdvanced ? getOverspending(user.id) : Promise.resolve(null),
    canAdvanced ? getSmartAlerts(user.id) : Promise.resolve([]),
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Onde estou gastando demais (avançado) */}
      {canAdvanced && overspending ? (
        <Card>
                ...
                {overspending.items.map((item) => (
                  ...
                  >
                    ...
                    <p className="mt-3 text-sm text-amber-900">
                      ...
                      {formatCents(Math.round(item.differenceCents / 2))} por
                      mês, poderá juntar{" "}
                      <strong>
                        {formatCents(Math.round(item.differenceCents / 2) * 12)}
                      </strong>{" "}
                      em um ano.
                    </p>
                  </div>
      ...
      {/* Simulador — disponível em todos os planos */}
      <SavingsSimulator goals={goals} />

      {/* Posso comprar (Básico/Pro) */}
      {canPurchase ? (
        <PurchaseAnalyzer
        ...
        <LockedFeature feature="PURCHASE_ANALYSIS" title="Posso comprar?" />
      )}
      ...
      {canAI ? (
        ...
        <LockedFeature feature="AI_ASSISTANT" title="Assistente com IA" />
      )}
    </div>
  );
}
